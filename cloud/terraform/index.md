# Terraform

[Back](../../index.md)

---

?????
provider
dynamic blocks
module, source, versioning handling
terraform graph
lifecycle block
rolling update: create_before_destroy = true + lifecycle block
null_resource / terraform_data
terratest
apply -target
local-exec / remote-exec

## HCL Terraform


- [Common Commands](./command/command.md)
- [Interpolation](./Interpolation/Interpolation.md)
- [Project Structure](./structure/structure.md)
- [Lockfile](./lockfile/lockfile.md)
- [Packer](./packer/packer.md)
- [Module](./module/module.md)
- [IaC](./iac/lac.md)
- [Terraform CLI](./cli/cli.md)
- [Resource & Data source](./resource_data/resource_data.md)
- CDKTF
  - [with AWS](./aws/aws.md)

- [Fundamental](./fundamental/fundamental.md)

- Blocks
- Arguments
- Expressions
  - [Resource](./resource/resource.md)
  - [Data Sources](./data_source/data_source.md)
  - [Variable](./variable/variable.md)
  - [Function](./function/function.md)
  - [Loop](./loop/loop.md)

- Workflow
  - [init](./workflow/init.md)
  - [Common Workflow](./workflow/workflow.md)
  - [Workspace](./workflow/workspace.md)

- State Managetment
  - [State](./state/state.md)
  - [Backend](./state/state_backend.md)
  - [Import](./state/state_import.md)

---

## AWS

- [VPC](./aws/vpc/vpc.md)
- [EC2](./aws/ec2/ec2.md)
- [RDS](./aws/rds/rds.md)
- [IAM](./aws/iam/iam.md)
- [ECS](./aws/ecs/ecs.md)
