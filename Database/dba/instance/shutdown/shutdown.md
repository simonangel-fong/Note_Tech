...
- [DBA: Instance - Shutdown](#dba-instance---shutdown)
  - [Instance and Database Shutdown](#instance-and-database-shutdown)
    - [Phase: `Close`](#phase-close)
      - [Normal Shutdown - `IMMEDIATE`, `TRANSACTIONAL`, `NORMAL`,](#normal-shutdown---immediate-transactional-normal)
  ...
  - [Shutdown Modes](#shutdown-modes)
    - [`SHUTDOWN ABORT`](#shutdown-abort)
      - [vs Other mode(save shutdown)](#vs-other-modesave-shutdown)
...
#### Normal Shutdown - `IMMEDIATE`, `TRANSACTIONAL`, `NORMAL`,
...
- Oracle Database does **not write data** in the buffers of the SGA to
...
- **terminates** any executing SQL statements
