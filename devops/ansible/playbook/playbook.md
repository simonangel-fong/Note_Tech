...
- Optional Directives:
  - `become`
    ...
    - Forces the specific task to **run with administrative root privileges**, overriding or following play-level constraints.
  - `when`
    - Conditionals
    - Controls execution logic.
    - The task runs only if the provided **Jinja2 expression** resolves to `true`.
  - `loop` / `with_<lookup>`
    - Loops
    - **Repeats** the task iteration sequence over an array or dictionary of values.
  - `register`
    - Variable Capturing
    - Saves the **output** results, **status** flags, and **error** state logs of the task directly into a custom variable for downstream use.
  - `notify`
    - Handler Triggers
